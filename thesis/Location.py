...
class Placement(nn.Module):
    ...
    def Locate(self,grids,blocks,masks,ans):
        ...
        print(masks)
        print(ans)
        b,r,c,f=grids.shape # batch, r, c, fea->time, weight1, weight2,  # blocks b, fea->time, weight1, weight2,
        #blocks b, lookahead*featuredim
        ones_grids = torch.ones((b, r, c, 1),device='cuda')
        grids = torch.cat([grids, ones_grids], axis=-1)
        b, r, c, f = grids.shape

        blocks=blocks.reshape(b,-1)
        ones_blocks = torch.ones((b, 1),device='cuda')
        blocks=torch.cat([blocks,ones_blocks],axis=-1)
